...
import { APP_NAME, cloneJSON, EVENT, toBrandedType } from "@hidewhiteboard/common";
import {
  IDLE_THRESHOLD,
  ACTIVE_THRESHOLD,
  UserIdleState,
...
import { decryptData } from "@hidewhiteboard/hidewhiteboard/data/encryption";
import { getVisibleSceneBounds } from "@hidewhiteboard/element";
import { newElementWith } from "@hidewhiteboard/element";
import { isImageElement, isInitializedImageElement } from "@hidewhiteboard/element";
import { AbortError } from "@hidewhiteboard/hidewhiteboard/errors";
import { t } from "@hidewhiteboard/hidewhiteboard/i18n";
import { withBatchedUpdates } from "@hidewhiteboard/hidewhiteboard/reactUtils";

import throttle from "lodash.throttle";
import { PureComponent } from "react";

import { bumpElementVersions } from "@hidewhiteboard/hidewhiteboard/data/restore";

import type {
  ReconciledhidewhiteboardElement,
  RemotehidewhiteboardElement,
} from "@hidewhiteboard/hidewhiteboard/data/reconcile";
import type { ImportedDataState } from "@hidewhiteboard/hidewhiteboard/data/types";
import type {
...
class Collab extends PureComponent<CollabProps, CollabState> {
  ...
  hidewhiteboardAPI: CollabProps["hidewhiteboardAPI"];
  ...
  constructor(props: CollabProps) {
    ...
    this.hidewhiteboardAPI = props.hidewhiteboardAPI;
  ...
  componentDidMount() {
    ...
    const unsubOnUserFollow = this.hidewhiteboardAPI.onUserFollow((payload) => {
      this.portal.socket && this.portal.broadcastUserFollowed(payload);
    });
    const throttledRelayUserViewportBounds = throttleRAF(
      this.relayVisibleSceneBounds,
    );
    const unsubOnScrollChange = this.hidewhiteboardAPI.onScrollChange(() =>
  ...
  saveCollabRoomToFirebase = async (
    syncableElements: readonly SyncablehidewhiteboardElement[],
  ) => {
    ...
    try {
      const storedElements = await saveToFirebase(
        this.portal,
        syncableElements,
        this.hidewhiteboardAPI.getAppState(),
      );
  ...
  stopCollaboration = (keepRemoteState = true) => {
    ...
    this.saveCollabRoomToFirebase(
      getSyncableElements(
        this.hidewhiteboardAPI.getSceneElementsIncludingDeleted(),
      ),
    );
    ...
    if (!keepRemoteState) {
      ...
      const elements = this.hidewhiteboardAPI
      ...
      this.hidewhiteboardAPI.updateScene({
        elements,
  ...
  private fetchImageFilesFromFirebase = async (opts: {
    ...
    const unfetchedImages = opts.elements
      ...
      .map((element) => (element as InitializedhidewhiteboardImageElement).fileId);
  ...
  startCollaboration = async (
    ...
    if (!this.state.username) {
      import("@hidewhiteboard/random-username").then(({ getRandomUsername }) => {
    ...
    const scenePromise = resolvablePromise<
      | (ImportedDataState & { elements: readonly OrderedhidewhiteboardElement[] })
      | null
    >();
    ...
    if (existingRoomLinkData) {
      ...
      this.hidewhiteboardAPI.resetScene();
    } else {
      const elements = this.hidewhiteboardAPI.getSceneElements().map((element) => {
        if (isImageElement(element) && element.status === "saved") {
          return newElementWith(element, { status: "pending" });
        }
        return element;
      ...
      async (encryptedData: ArrayBuffer, iv: Uint8Array<ArrayBuffer>) => {
        ...
        switch (decryptedData.type) {
          ...
          case WS_SUBTYPES.INIT: {
            if (!this.portal.socketInitialized) {
              ...
              const remoteElements = toBrandedType<
                readonly RemotehidewhiteboardElement[]
          ...
          case WS_SUBTYPES.UPDATE:
            this.handleRemoteSceneUpdate(
              this._reconcileElements(
                toBrandedType<readonly RemotehidewhiteboardElement[]>(
                  decryptedData.payload.elements,
                ),
              ),
            );
          ...
          case WS_SUBTYPES.USER_VISIBLE_SCENE_BOUNDS: {
            ...
            const appState = this.hidewhiteboardAPI.getAppState();
  ...
  private _reconcileElements = (
    remoteElements: readonly RemotehidewhiteboardElement[],
  ): ReconciledhidewhiteboardElement[] => {
    const appState = this.hidewhiteboardAPI.getAppState();
  ...
  private loadImageFiles = throttle(async () => {
    const { loadedFiles, erroredFiles } =
      await this.fetchImageFilesFromFirebase({
        elements: this.hidewhiteboardAPI.getSceneElementsIncludingDeleted(),
      });

    this.hidewhiteboardAPI.addFiles(loadedFiles);

    updateStaleImageStatuses({
      hidewhiteboardAPI: this.hidewhiteboardAPI,
      erroredFiles,
      elements: this.hidewhiteboardAPI.getSceneElementsIncludingDeleted(),
    });
  }, LOAD_IMAGES_TIMEOUT);
  ...
  private handleRemoteSceneUpdate = (
    elements: ReconciledhidewhiteboardElement[],
  ) => {
    this.hidewhiteboardAPI.updateScene({
      elements,
      captureUpdate: CaptureUpdateAction.NEVER,
  ...
  setCollaborators(sockets: SocketId[]) {
    ...
    this.hidewhiteboardAPI.updateScene({ collaborators });
  }
  ...
  public getSceneElementsIncludingDeleted = () => {
    return this.hidewhiteboardAPI.getSceneElementsIncludingDeleted();
  };
  ...
  broadcastElements = (elements: readonly OrderedhidewhiteboardElement[]) => {
    if (
      getSceneVersion(elements) >
  ...
  syncElements = (elements: readonly OrderedhidewhiteboardElement[]) => {
