import { reconcileElements } from "@hidewhiteboard/hidewhiteboard";
import { MIME_TYPES, toBrandedType } from "@hidewhiteboard/common";
import { decompressData } from "@hidewhiteboard/hidewhiteboard/data/encode";
import {
  encryptData,
  decryptData,
} from "@hidewhiteboard/hidewhiteboard/data/encryption";
import { restoreElements } from "@hidewhiteboard/hidewhiteboard/data/restore";
import { getSceneVersion } from "@hidewhiteboard/element";
...
import type { SyncablehidewhiteboardElement } from ".";
...
const decryptElements = async (
  ...
  roomKey: string,
): Promise<readonly hidewhiteboardElement[]> => {
...
class FirebaseSceneVersionCache {
  ...
  static set = (
    socket: Socket,
    elements: readonly SyncablehidewhiteboardElement[],
  ) => {
...
export const saveToFirebase = async (
  ...
  const storedScene = await runTransaction(firestore, async (transaction) => {
    ...
    const reconciledElements = getSyncableElements(
      reconcileElements(
        elements,
        prevStoredElements as OrderedhidewhiteboardElement[] as RemotehidewhiteboardElement[],
        appState,
      ),
    );
  ...
  return toBrandedType<RemotehidewhiteboardElement[]>(storedElements);
};
...
export const loadFromFirebase = async (
  ...
  roomKey: string,
  socket: Socket | null,
): Promise<readonly SyncablehidewhiteboardElement[] | null> => {
  const firestore = _getFirestore();
  const docRef = doc(firestore, "scenes", roomId);
