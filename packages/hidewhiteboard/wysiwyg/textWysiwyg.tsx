...
import { getTextWidth } from "@hidewhiteboard/element";
import { normalizeText } from "@hidewhiteboard/element";
...
export const textWysiwyg = ({
  ...
  hidewhiteboardContainer: HTMLDivElement | null;
  app: App;
  autoSelect?: boolean;
}): SubmitHandler => {
  const textPropertiesUpdated = (
    updatedTextElement: hidewhiteboardTextElement,
  ...
  const updateWysiwygStyle = () => {
    ...
    const updatedTextElement = app.scene.getElement<hidewhiteboardTextElement>(id);
        ...
        if (isArrowElement(container)) {
          const boundTextCoords =
            LinearElementEditor.getBoundTextElementPosition(
              container,
              updatedTextElement as hidewhiteboardTextElementWithContainer,
              elementsMap,
            );
  ...
  editable.wrap = "off";
  editable.classList.add("hidewhiteboard-wysiwyg");
  ...
  if (onChange) {
    editable.onpaste = async (event) => {
      ...
      // when copy/pasting hidewhiteboard elements, only paste the text content
      //
      ...
      // parse the text/plain for existence of hidewhiteboard instead, but this
      // is an edge case
      if (
        mimeTypes.has(MIME_TYPES.hidewhiteboardClipboard) ||
        mimeTypes.has(MIME_TYPES.hidewhiteboard)
      ) {
        ...
        try {
          ...
          // if hidewhiteboard elements don't contain any text elements,
          // don't paste anything
          return;
        } catch {
          console.warn("failed to parse hidewhiteboard clipboard data");
        }
      }
      ...
      if (editable.value.trim()) {
        ...
        app.scene.mutateElement(container, {
          boundElements: container.boundElements?.filter(
            (ele) =>
              !isTextElement(
                ele as hidewhiteboardTextElement | hidewhiteboardLinearElement,
              ),
          ),
        });
      }
  ...
  hidewhiteboardContainer
    ?.querySelector(".hidewhiteboard-textEditorContainer")!
