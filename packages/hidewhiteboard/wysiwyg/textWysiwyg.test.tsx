...
import { pointFrom } from "@hidewhiteboard/math";
import { getOriginalContainerHeightFromCache } from "@hidewhiteboard/element";
...
describe("textWysiwyg", () => {
  describe("start text editing", () => {
    const { h } = window;
    beforeEach(async () => {
      await render(<Hidewhiteboard handleKeyboardGlobally={true} />);
      API.setElements([]);
    });
    ...
    it("should prefer editing selected text element (non-bindable container present)", async () => {
      ...
      expect(
        (h.state.editingTextElement as hidewhiteboardTextElement).containerId,
      ).toBe(null);
    });
  ...
  describe("Test text wrapping", () => {
    ...
    it("should restore original text after unwrapping a wrapped text", async () => {
      const originalText = "hidewhiteboard\neditor\nis great!";
      const text = API.createElement({
        type: "text",
        text: originalText,
      });
      ...
      UI.resize(text, "e", [40, 0]);
      expect((h.elements[0] as hidewhiteboardTextElement).text).toBe(originalText);
  ...
  describe("Test container-unbound text", () => {
    ...
    let textElement: hidewhiteboardTextElement;
    ...
    it("text should never go beyond max width", async () => {
      ...
      updateTextEditor(
        textarea,
        "hidewhiteboard is an opensource virtual collaborative whiteboard for sketching hand-drawn like diagrams!",
      );
  ...
  describe("Test container-bound text", () => {
    ...
    it("should bind text to container when double clicked inside filled container", async () => {
      ...
      const text = h.elements[1] as hidewhiteboardTextElementWithContainer;
    ...
    it("should bind text to container when double clicked inside of the transparent container", async () => {
      ...
      mouse.doubleClickAt(rectangle.x + 10, rectangle.y + 10);
      expect(h.elements.length).toBe(2);
      let text = h.elements[1] as hidewhiteboardTextElementWithContainer;
      expect(text.type).toBe("text");
      ...
      text = h.elements[1] as hidewhiteboardTextElementWithContainer;
      expect(text.type).toBe("text");
      expect(text.containerId).toBe(rectangle.id);
    ...
    it("shouldn't bind to non-text-bindable containers", async () => {
      ...
      expect((h.elements[1] as hidewhiteboardTextElement).containerId).toBe(null);
    });
    ...
    it("should bind text to container when triggered via context menu", async () => {
      ...
      updateTextEditor(
        editor,
        "hidewhiteboard is an opensource virtual collaborative whiteboard",
      );
      expect(h.elements.length).toBe(2);
      expect(h.elements[1].type).toBe("text");
    ...
    it("should update font family correctly on undo/redo by selecting bounded text when font family was updated", async () => {
      ...
      expect(
        (h.elements[1] as hidewhiteboardTextElementWithContainer).fontFamily,
    ...
    it("should unbind bound text when unbind action from context menu is triggered", async () => {
      ...
      expect((h.elements[1] as hidewhiteboardTextElement).containerId).toEqual(
        null,
      );
    });
    ...
    it("should scale font size correctly when resizing using shift", async () => {
      ...
      updateTextEditor(editor, "Hello");
      Keyboard.exitTextEditor(editor);
      const textElement = h.elements[1] as hidewhiteboardTextElement;
    ...
    it("should bind text correctly when container duplicated with alt-drag", async () => {
      ...
      const duplicatedText = h
        .elements[1] as hidewhiteboardTextElementWithContainer;
      const originalRect = h.elements[2];
      const originalText = h.elements[3] as hidewhiteboardTextElementWithContainer;
    ...
    it("should restore original container height and clear cache once text is unbind", async () => {
      ...
      expect((h.elements[1] as hidewhiteboardTextElementWithContainer).text).toBe(
        "Online\nwhiteboa\nrd\ncollabor\nation\nmade\neasy",
      );
      fireEvent.contextMenu(GlobalTestState.interactiveCanvas, {
        button: 2,
    ...
    it("should reset the container height cache when font properties updated", async () => {
      ...
      expect(
        (h.elements[1] as hidewhiteboardTextElementWithContainer).fontSize,
      ).toEqual(36);
    ...
    it("should update line height when font family updated", async () => {
      ...
      Keyboard.exitTextEditor(editor);
      expect(
        (h.elements[1] as hidewhiteboardTextElementWithContainer).lineHeight,
      ).toEqual(1.25);
    ...
    it("should wrap text in a container when wrap text in container triggered from context menu", async () => {
      ...
      expect((textElement as hidewhiteboardTextElement).text).toBe(
        "hidewhiteboard is an opensource virtual collaborative whiteboard",
      );
      ...
      expect(h.elements[2] as hidewhiteboardTextElement).toEqual(
        expect.objectContaining({
          text: "hidewhiteboard is an opensource virtual collaborative whiteboard",
    ...
    it("shouldn't bind to container if container has bound text not centered and text tool is used", async () => {
      ...
      updateTextEditor(editor, "Hello!");
      expect(
        (h.elements[1] as hidewhiteboardTextElementWithContainer).verticalAlign,
      ...
      editor = await getTextEditor();
      updateTextEditor(editor, "hidewhiteboard");
      Keyboard.exitTextEditor(editor);
      ...
      text = h.elements[2] as hidewhiteboardTextElementWithContainer;
      expect(text.containerId).toBe(null);
      expect(text.text).toBe("hidewhiteboard");
    });
  ...
  describe("Test theme change", () => {
    ...
    it("should update textarea color when theme changes to dark mode and back", async () => {
      ...
      mouse.doubleClickOn(textElement as hidewhiteboardTextElement);
