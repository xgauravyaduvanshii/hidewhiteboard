...
import { mutateElement } from "@hidewhiteboard/element";
import { deepCopyElement } from "@hidewhiteboard/element";
import {
  isFrameLikeElement,
  isInitializedImageElement,
} from "@hidewhiteboard/element";

import { getContainingFrame } from "@hidewhiteboard/element";

import type { ValueOf } from "@hidewhiteboard/common/utility-types";

import type { IMAGE_MIME_TYPES, STRING_MIME_TYPES } from "@hidewhiteboard/common";
import type {
  hidewhiteboardElement,
  NonDeletedhidewhiteboardElement,
} from "@hidewhiteboard/element/types";

import { hidewhiteboardError } from "./errors";
import {
  createFile,
  getFileHandle,
...
type ElementsClipboard = {
  type: typeof EXPORT_DATA_TYPES.hidewhiteboardClipboard;
...
const clipboardContainsElements = (
  contents: any,
): contents is { elements: hidewhiteboardElement[]; files?: BinaryFiles } => {
  if (
    [
      EXPORT_DATA_TYPES.hidewhiteboard,
      EXPORT_DATA_TYPES.hidewhiteboardClipboard,
      EXPORT_DATA_TYPES.hidewhiteboardClipboardWithAPI,
...
export const serializeAsClipboardJSON = ({
  ...
  const contents: ElementsClipboard = {
    type: EXPORT_DATA_TYPES.hidewhiteboardClipboard,
...
export const copyToClipboard = async (
  ...
  await copyTextToSystemClipboard(
    {
      [MIME_TYPES.hidewhiteboardClipboard]: json,
    ...
    for (const type of item.types) {
      ...
      try {
        if (type === MIME_TYPES.text || type === MIME_TYPES.html) {
          ...
          throw new hidewhiteboardError(`Unsupported clipboard type: ${type}`);
        }
      } catch (error: any) {
        console.warn(
...
export const parseClipboard = async (
  ...
  try {
    ...
    const programmaticAPI =
      systemClipboardData.type === EXPORT_DATA_TYPES.hidewhiteboardClipboardWithAPI;
