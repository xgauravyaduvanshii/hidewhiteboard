...
import { canBecomePolygon, getNonDeletedElements } from "@hidewhiteboard/element";
...
import { hasStrokeColor } from "@hidewhiteboard/element";
...
import { deriveStylesPanelMode } from "@hidewhiteboard/common";
...
import type { Scene } from "@hidewhiteboard/element";
...
export const changeProperty = (
  ...
  appState: AppState,
  callback: (element: hidewhiteboardElement) => hidewhiteboardElement,
...
export const getFormValue = function <T extends Primitive>(
  ...
  getAttribute: (element: hidewhiteboardElement) => T,
  isRelevantElement: true | ((element: hidewhiteboardElement) => boolean),
...
const offsetElementAfterFontResize = (
  prevElement: hidewhiteboardTextElement,
  nextElement: hidewhiteboardTextElement,
  scene: Scene,
) => {
  ...
  return scene.mutateElement(nextElement, {
    ...
    // centering vertically is non-standard, but for hidewhiteboard I think
    // it makes sense
...
const changeFontSize = (
  ...
  appState: AppState,
  app: AppClassProperties,
  getNewFontSize: (element: hidewhiteboardTextElement) => number,
  fallbackValue?: hidewhiteboardTextElement["fontSize"],
) => {
  ...
  const updatedElements = changeProperty(
    ...
    (oldElement) => {
      if (isTextElement(oldElement)) {
        ...
        let newElement: hidewhiteboardTextElement = newElementWith(oldElement, {
...
export const actionChangeFillStyle = register<hidewhiteboardElement["fillStyle"]>({
...
export const actionChangeStrokeWidth = register<
  hidewhiteboardElement["strokeWidth"]
>({
...
export const actionChangeSloppiness = register<hidewhiteboardElement["roughness"]>({
...
export const actionChangeStrokeStyle = register<
  hidewhiteboardElement["strokeStyle"]
>({
...
export const actionChangeOpacity = register<hidewhiteboardElement["opacity"]>({
  name: "changeOpacity",
...
export const actionChangeFontSize = register<hidewhiteboardTextElement["fontSize"]>(
  {
    name: "changeFontSize",
...
export const actionChangeFontFamily = register<{
    ...
    if (currentItemFontFamily) {
      ...
      if (selectedTextElements.length > 200) {
        ...
        while (
          ...
          const textElement = selectedTextElements[i] as hidewhiteboardTextElement;
        ...
        elements: changeProperty(
          ...
          (oldElement) => {
            if (
              ...
              const newElement: hidewhiteboardTextElement = newElementWith(
                oldElement,
                {
        ...
        window.document.fonts.load(fontString, chars).then((fontFaces) => {
          for (const [element, container] of elementContainerMapping) {
            ...
            if (latestElement) {
              ...
              redrawTextBoundingBox(
                latestElement as hidewhiteboardTextElement,
                latestContainer,
                app.scene,
              );
            }
  ...
  PanelComponent: ({ elements, appState, app, updateData }) => {
    ...
    const selectedFontFamily = useMemo(() => {
      const getFontFamily = (
        elementsArray: readonly hidewhiteboardElement[],
    ...
    return {
      elements: changeProperty(elements, appState, (el) => {
        if (isLinearElement(el)) {
          ...
          if (position === "start") {
            const element: hidewhiteboardLinearElement = newElementWith(el, {
              startArrowhead: type,
            });
            return element;
    ...
    if (selectedId) {
      ...
      if (selected) {
        newState.selectedLinearElement = new LinearElementEditor(
          selected as hidewhiteboardLinearElement,
          arrayToMap(elements),
        );
      }
    }
