...
import { hasBoundTextElement, isTextElement } from "@hidewhiteboard/element";
...
const getApplicableTextElements = (
  ...
  elements.reduce(
    (acc: hidewhiteboardTextElement[], el) => {
      if (!el || isInGroup(el)) {
        return acc;
      }
      if (isTextElement(el)) {
...
> = ({
  ...
  if (nextValue) {
    ...
    const originalTextElements = originalElements as hidewhiteboardTextElement[];
