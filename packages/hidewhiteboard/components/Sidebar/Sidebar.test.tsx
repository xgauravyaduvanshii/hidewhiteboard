...
import { DEFAULT_SIDEBAR } from "@hidewhiteboard/common";

import { Hidewhiteboard, Sidebar } from "../../index";
import {
  act,
  fireEvent,
  queryAllByTestId,
...
import {
  asserthidewhiteboardWithSidebar,
...
describe("Sidebar", () => {
  describe("General behavior", () => {
    ...
    it("should toggle sidebar using hidewhiteboardAPI.toggleSidebar()", async () => {
      const { container } = await render(
        <Hidewhiteboard>
          <Sidebar name="customSidebar">
            <div id="test-sidebar-content">42</div>
  ...
  describe("Docking behavior", () => {
    it("shouldn't be user-dockable if `onDock` not supplied", async () => {
      await asserthidewhiteboardWithSidebar(
        <Sidebar name="customSidebar">
          <Sidebar.Header />
        </Sidebar>,
        "customSidebar",
    ...
    it("should be user-dockable when both `onDock` and `docked` supplied", async () => {
      ...
      await withhidewhiteboardDimensions(
