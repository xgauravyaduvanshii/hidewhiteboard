...
import { actionToggleShapeSwitch } from "@hidewhiteboard/hidewhiteboard/actions/actionToggleShapeSwitch";

import { getShortcutKey } from "@hidewhiteboard/hidewhiteboard/shortcut";

import type { MarkRequired } from "@hidewhiteboard/common/utility-types";
...
import {
  useApp,
  useAppProps,
  usehidewhiteboardActionManager,
  usehidewhiteboardSetAppState,
} from "../App";
...
function CommandPaletteInner({
  ...
  const actionManager = usehidewhiteboardActionManager();
  ...
  useEffect(() => {
    ...
    if (uiAppState && app.scene && actionManager) {
      ...
      const additionalCommands: CommandPaletteItem[] = [
        ...
        {
          label: `${t("toolBar.mermaidTohidewhiteboard")}...`,
          category: DEFAULT_CATEGORIES.tools,
  ...
  const executeCommand = (
    ...
    if (uiAppState.openDialog?.name === "commandPalette") {
      event.stopPropagation();
      event.preventDefault();
      document.body.classList.add("hidewhiteboard-animations-disabled");
      closeCommandPalette(() => {
        command.perform({ actionManager, event });
        setLastUsed(command);

        requestAnimationFrame(() => {
          document.body.classList.remove("hidewhiteboard-animations-disabled");
        });
      });
    }
  };
