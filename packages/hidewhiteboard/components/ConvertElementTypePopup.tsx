...
import { pointFrom, pointRotateRads, type LocalPoint } from "@hidewhiteboard/math";
...
import { wrapText } from "@hidewhiteboard/element";
...
import { ShapeCache } from "@hidewhiteboard/element";

import { updateBindings } from "@hidewhiteboard/element";
...
type hidewhiteboardConvertibleElement =
  | hidewhiteboardRectangleElement
  | hidewhiteboardDiamondElement
  | hidewhiteboardEllipseElement
  | hidewhiteboardLinearElement;
...
export const adjustBoundTextSize = (
  container: hidewhiteboardTextContainer,
  boundText: hidewhiteboardTextElementWithContainer,
  scene: Scene,
) => {
...
export const convertElementTypes = (
  ...
  if (conversionType === "generic") {
    ...
    if (nextType && isConvertibleGenericType(nextType)) {
      const convertedElements: Record<string, hidewhiteboardElement> = {};
      ...
      for (const element of Object.values(convertedElements)) {
        ...
        if (boundText) {
          ...
          adjustBoundTextSize(
            element as hidewhiteboardTextContainer,
            boundText,
            app.scene,
          );
        }
  ...
  if (conversionType === "linear") {
    const convertibleLinearElements = filterLinearConvertibleElements(
      selectedElements,
    ) as hidewhiteboardLinearElement[];
    ...
    if (isConvertibleLinearType(nextType)) {
      const convertedElements: hidewhiteboardElement[] = [];

      const nextElementsMap: Map<hidewhiteboardElement["id"], hidewhiteboardElement> =
...
const isEligibleLinearElement = (element: hidewhiteboardElement) => {
  return (
...
const toCacheKey = (
  elementId: hidewhiteboardElement["id"],
  convertitleType: ConvertibleTypes,
) => {
  return `${elementId}:${convertitleType}` as CacheKey;
};

const filterGenericConvetibleElements = (elements: hidewhiteboardElement[]) =>
...
const filterLinearConvertibleElements = (elements: hidewhiteboardElement[]) =>
...
const convertLineToElbow = (line: hidewhiteboardLinearElement): LocalPoint[] => {
...
const convertElementType = <
  TElement extends Exclude<hidewhiteboardElement, hidewhiteboardSelectionElement>,
>(
  element: TElement,
...
const getConvertibleType = (
  element: hidewhiteboardConvertibleElement,
): ConvertibleTypes => {
  if (isLinearElement(element)) {
