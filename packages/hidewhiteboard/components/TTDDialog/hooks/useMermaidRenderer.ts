...
import { convertMermaidTohidewhiteboard } from "../common";
...
export const useMermaidRenderer = ({
  ...
  const renderMermaid = useCallback(
    async (mermaidDefinition: string): Promise<boolean> => {
      if (!mermaidDefinition.trim() || !mermaidTohidewhiteboardLib.loaded) {
        return false;
      }
    ...
    [canvasRef, mermaidTohidewhiteboardLib, setError, theme],
  );
