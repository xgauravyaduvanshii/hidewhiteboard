import { pointFrom, type GlobalPoint } from "@hidewhiteboard/math";
...
import { EVENT, HYPERLINK_TOOLTIP_DELAY, KEYS } from "@hidewhiteboard/common";

import { getElementAbsoluteCoords } from "@hidewhiteboard/element";

import { hitElementBoundingBox } from "@hidewhiteboard/element";

import { isElementLink } from "@hidewhiteboard/element";

import { getEmbedLink, embeddableURLValidator } from "@hidewhiteboard/element";
...
import { isEmbeddableElement } from "@hidewhiteboard/element";
...
import { useAppProps, useEditorInterface, usehidewhiteboardAppState } from "../App";
...
import type { AppState, hidewhiteboardProps, UIAppState } from "../../types";
...
const embeddableLinkCache = new Map<
  hidewhiteboardEmbeddableElement["id"],
  string
>();
...
export const Hyperlink = ({
  ...
  onLinkOpen: hidewhiteboardProps["onLinkOpen"];
  setToast: (
  ...
  return (
    <div
      className="hidewhiteboard-hyperlinkContainer"
      style={{
        top: `${y}px`,
        left: `${x}px`,
        width: POPUP_WIDTH,
    ...
    >
      {isEditing ? (
        <input
          className={clsx("hidewhiteboard-hyperlinkContainer-input")}
        ...
        <a
          ...
          className="hidewhiteboard-hyperlinkContainer-link"
          target={isLocalLink(element.link) ? "_self" : "_blank"}
          onClick={(event) => {
            if (element.link && onLinkOpen) {
              const customEvent = wrapEvent(
                EVENT.HIDEWHITEBOARD_LINK,
                event.nativeEvent,
              );
              onLinkOpen(
                {
        ...
        <div className="hidewhiteboard-hyperlinkContainer-link">
          {t("labels.link.empty")}
        </div>
      )}
      <div className="hidewhiteboard-hyperlinkContainer__buttons">
        {!isEditing && (
          <ToolButton
            type="button"
            ...
            onClick={onEdit}
            className="hidewhiteboard-hyperlinkContainer--edit"
            icon={FreedrawIcon}
          />
        )}
        <ToolButton
        ...
        {linkVal && !isEmbeddableElement(element) && (
          <ToolButton
            ...
            className="hidewhiteboard-hyperlinkContainer--remove"
            icon={TrashIcon}
          />
        )}
      </div>
...
const renderTooltip = (
  ...
  tooltipDiv.classList.add("hidewhiteboard-tooltip--visible");
...
export const hideHyperlinkToolip = () => {
  ...
  if (IS_HYPERLINK_TOOLTIP_VISIBLE) {
    ...
    getTooltipDiv().classList.remove("hidewhiteboard-tooltip--visible");
  }
};
