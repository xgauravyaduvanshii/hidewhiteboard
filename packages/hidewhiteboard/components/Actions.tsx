...
export const canChangeStrokeColor = (
  appState: UIAppState,
  targetElements: hidewhiteboardElement[],
) => {
  let commonSelectedType: hidewhiteboardElementType | null =
...
export const CompactShapeActions = ({
  ...
  const targetElements = getTargetElements(elementsMap, appState);
  const { container } = usehidewhiteboardContainer();
...
export const ShapesSwitcher = ({
  ...
  const isToolEnabled = (tool: string) =>
    UIOptions.tools?.[
      tool as Extract<keyof NonNullable<AppProps["UIOptions"]["tools"]>, string>
    ] !== false;

  const handAndSelectionTools = [
    ...(isToolEnabled("selection")
      ? [
          {
            type: "selection",
            icon: SelectionIcon,
      ...
      : []),
    {
      type: "hand",
      icon: handIcon,
      title: capitalizeString(t("toolBar.hand")),
    },
  ] as const;

  type ShapePopoverToolType =
    | "rectangle"
    | "diamond"
    | "ellipse"
    | "arrow"
  ...
  const shapeTools = [
    ...
    (tool): tool is {
      ...
      title: string;
    } => !!tool.icon && isToolEnabled(tool.type),
  );

  const displayedHandSelectionTool =
    handAndSelectionTools.find((tool) => tool.type === activeTool.type) ||
    handAndSelectionTools[0];

  const displayedShapeTool =
    shapeTools.find((tool) => tool.type === activeTool.type) || shapeTools[0];

  return (
    <>
      {handAndSelectionTools.length > 0 && displayedHandSelectionTool && (
        <ToolPopover
          app={app}
          options={handAndSelectionTools}
          activeTool={activeTool}
          defaultOption={displayedHandSelectionTool.type}
          namePrefix="selectionAndHand"
          title={capitalizeString(t("toolBar.selection"))}
          ...
          displayedOption={displayedHandSelectionTool}
          fillable={activeTool.type === "selection"}
        />
      )}

      {shapeTools.length > 0 && displayedShapeTool && (
        <ToolPopover
          app={app}
          options={shapeTools}
          activeTool={activeTool}
          defaultOption={displayedShapeTool.type}
          namePrefix="shapeType"
          title={capitalizeString(t("toolBar.rectangle"))}
          data-testid="toolbar-shape-group"
          onToolChange={(type: string) => {
            if (
              type === "rectangle" ||
              type === "diamond" ||
          ...
          displayedOption={displayedShapeTool}
          fillable={
      ...
      {getToolbarTools(app).map(({ value, icon, key, numericKey, fillable }) => {
        if (
          !isToolEnabled(value) ||
          value === "selection" ||
          value === "lasso" ||
      ...
      <DropdownMenu open={isExtraToolsMenuOpen}>
        ...
        >
          ...
          >
            {t("toolBar.mermaidTohidewhiteboard")}
          </DropdownMenu.Item>
