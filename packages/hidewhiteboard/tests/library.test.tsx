...
import { MIME_TYPES, ORIG_ID } from "@hidewhiteboard/common";

import { getCommonBoundingBox } from "@hidewhiteboard/element";

import type { hidewhiteboardGenericElement } from "@hidewhiteboard/element/types";
...
const libraryJSONPromise = API.readFile(
  "./fixtures/fixture_library.hidewhiteboardlib",
  "utf8",
);
...
const mockLibraryFilePromise = new Promise<Blob>(async (resolve, reject) => {
  try {
    resolve(
      new Blob([await libraryJSONPromise], { type: MIME_TYPES.hidewhiteboardlib }),
    );
  } catch (error) {
    reject(error);
  }
...
describe("library items inserting", () => {
  beforeEach(async () => {
    ...
    await render(<Hidewhiteboard initialData={{ libraryItems }} />);
  });
    ...
    await API.drop([
      {
        ...
        type: MIME_TYPES.hidewhiteboardlibIds,
      },
    ]);
    ...
    await API.drop([
      {
        ...
        file: await API.loadFile("./fixtures/fixture_library.hidewhiteboardlib"),
      },
    ]);
    await waitFor(async () => {
      expect(await h.app.library.getLatestLibrary()).toEqual([
...
describe("distributeLibraryItemsOnSquareGrid()", () => {
  it("should distribute items on a grid", async () => {
    const createLibraryItem = (
      elements: hidewhiteboardGenericElement[],
    ): LibraryItem => {
      return {
        id: `id-${Date.now()}`,
        elements,
