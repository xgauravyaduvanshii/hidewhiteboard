import { pointFrom, pointRotateRads } from "@hidewhiteboard/math";
...
import { cropElement } from "@hidewhiteboard/element";
import {
  getTransformHandles,
...
import { KEYS, arrayToMap } from "@hidewhiteboard/common";

import type { GlobalPoint, LocalPoint, Radians } from "@hidewhiteboard/math";

import type { TransformHandleType } from "@hidewhiteboard/element";
import type {
...
export class Pointer {
  ...
  select(
    ...
    elements: hidewhiteboardElement | hidewhiteboardElement[],
  ) {
    API.clearSelection();
  ...
  clickOn(element: hidewhiteboardElement) {
    this.reset();
    this.click(
      ...getElementPointForSelection(
        element,
  ...
  doubleClickOn(element: hidewhiteboardElement) {
    this.reset();
    this.doubleClick(
      ...getElementPointForSelection(
        element,
...
const transform = (
  element: hidewhiteboardElement | hidewhiteboardElement[],
...
const proxy = <T extends hidewhiteboardElement>(
  element: T,
): typeof element & {
      ...
      get(target, prop) {
        ...
        if (prop === "get") {
          if (currentElement.hasOwnProperty("get")) {
            throw new Error(
              "trying to get `get` test property, but hidewhiteboardElement seems to define its own",
            );
          }
...
export class UI {
  ...
  /**
   * Creates an hidewhiteboard element, and returns a proxy that wraps it so that
  ...
  static async editText<
    T extends hidewhiteboardTextElement | hidewhiteboardTextContainer,
  ...
  static crop(
    element: hidewhiteboardImageElement,
    handle: TransformHandleDirection,
    naturalWidth: number,
  ...
  static group(elements: hidewhiteboardElement[]) {
  ...
  static ungroup(elements: hidewhiteboardElement[]) {
    mouse.select(elements);
    Keyboard.withModifierKeys({ ctrl: true, shift: true }, () => {
      Keyboard.keyPress(KEYS.G);
    });
