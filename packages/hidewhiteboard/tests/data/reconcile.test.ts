...
import { randomInteger, cloneJSON } from "@hidewhiteboard/common";
...
import type { RemotehidewhiteboardElement } from "../../data/reconcile";
...
type Cache = Record<string, hidewhiteboardElement | undefined>;
...
const test = <U extends `${string}:${"L" | "R"}`>(
  ...
  const reconciled = reconcileElements(
    cloneJSON(_local),
    cloneJSON(_remote) as RemotehidewhiteboardElement[],
    {} as AppState,
  );
  ...
  try {
    assert.deepEqual(
      reconcileElements(
        cloneJSON(_remote),
        cloneJSON(_local as RemotehidewhiteboardElement[]),
        {} as AppState,
      ).map((x) => x.id),
      reconciledIds,
  ...
  try {
    assert.deepEqual(
      reconcileElements(
        cloneJSON(_remote),
        cloneJSON(reconciled as unknown as RemotehidewhiteboardElement[]),
        {} as AppState,
      ).map((x) => x.id),
      reconciledIds,
  ...
  it("test identical elements reconciliation", () => {
    const testIdentical = (
      ...
      const ret = reconcileElements(
        local as unknown as OrderedhidewhiteboardElement[],
        remote as unknown as RemotehidewhiteboardElement[],
        {} as AppState,
      );
