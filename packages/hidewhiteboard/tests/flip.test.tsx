...
import { ROUNDNESS, KEYS, arrayToMap, cloneJSON } from "@hidewhiteboard/common";

import { pointFrom, type Radians } from "@hidewhiteboard/math";

import { getBoundTextElementPosition } from "@hidewhiteboard/element";
import { getElementAbsoluteCoords } from "@hidewhiteboard/element";
import { newLinearElement } from "@hidewhiteboard/element";
...
beforeEach(async () => {
  ...
  await render(<Hidewhiteboard autoFocus={true} handleKeyboardGlobally={true} />);
  API.setAppState({
    zoom: {
...
const checkElementsBoundingBox = async (
  element1: hidewhiteboardElement,
  element2: hidewhiteboardElement,
...
const checkTwoPointsLineHorizontalFlip = async () => {
  const originalElement = cloneJSON(h.elements[0]) as hidewhiteboardLinearElement;
  API.executeAction(actionFlipHorizontal);
  const newElement = h.elements[0] as hidewhiteboardLinearElement;
  await waitFor(() => {
...
describe("image", () => {
  ...
  it("flips an unrotated image horizontally correctly", async () => {
    ...
    await createImage();
    await waitFor(() => {
      expect((h.elements[0] as hidewhiteboardImageElement).scale).toEqual([1, 1]);
...
describe("mutliple elements", () => {
  it("with bound text flip correctly", async () => {
    ...
    const arrowText = h.elements[1] as hidewhiteboardTextElementWithContainer;
    const arrowTextPos = getBoundTextElementPosition(
      arrow.get(),
      arrowText,
      arrayToMap(h.elements),
    )!;
    const rectText = h.elements[3] as hidewhiteboardTextElementWithContainer;
