...
const querySearchInput = async () => {
  const input =
    h.app.hidewhiteboardContainerValue.container?.querySelector<HTMLInputElement>(
...
describe("search", () => {
  beforeEach(async () => {
    await render(<Hidewhiteboard handleKeyboardGlobally />);
    API.setAppState({
      openSidebar: null,
    });
  });
  ...
  it("should match text split across multiple lines", async () => {
    ...
    API.updateElement(h.elements[0] as hidewhiteboardTextElement, {
      text: "t\ne\ns\nt \nt\ne\nx\nt \ns\np\nli\nt \ni\nn\nt\no\nm\nu\nlt\ni\np\nl\ne \nli\nn\ne\ns",
  ...
  it("should match frame names", async () => {
    ...
    API.updateElement(h.elements[0] as hidewhiteboardFrameLikeElement, {
