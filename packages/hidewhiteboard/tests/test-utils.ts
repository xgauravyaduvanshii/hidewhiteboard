...
import { ORIG_ID, arrayToMap } from "@hidewhiteboard/common";
...
import type { AllPossibleKeys } from "@hidewhiteboard/common/utility-types";

import { STORAGE_KEYS } from "../../../hidewhiteboard-app/app_constants";
...
const renderApp: TestRenderFn = async (ui, options) => {
  ...
  Object.defineProperty(GlobalTestState, "canvas", {
    // must be a getter because at the time of hidewhiteboardApp render the
...
export const withhidewhiteboardDimensions = async (
...
export const assertSelectedElements = (
  ...elements: (
    | (hidewhiteboardElement["id"] | hidewhiteboardElement)[]
...
export const getCloneByOrigId = <T extends boolean = false>(
  origId: hidewhiteboardElement["id"],
  returnNullIfNotExists: T = false as T,
): T extends true ? hidewhiteboardElement | null : hidewhiteboardElement => {
  ...
  return null as T extends true ? hidewhiteboardElement | null : hidewhiteboardElement;
};
 ...
 */
export const assertElements = <T extends AllPossibleKeys<hidewhiteboardElement>>(
  actualElements: readonly hidewhiteboardElement[],
  /** array order matters */
  ...
  const selectedElementIds = expectedElementsWithIds.reduce(
    (acc: Record<hidewhiteboardElement["id"], true>, el) => {
      if (el.selected) {
        acc[el.id] = true;
      }
      return acc;
