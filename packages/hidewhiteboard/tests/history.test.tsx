...
import "@hidewhiteboard/utils/test-utils";

import { ElementsDelta, AppStateDelta } from "@hidewhiteboard/element";

import { CaptureUpdateAction, StoreDelta } from "@hidewhiteboard/element";
...
describe("history", () => {
  ...
  describe("singleplayer undo/redo", () => {
      ...
      await API.drop([
        {
          ...
          file: new Blob(
            [
              JSON.stringify({
                type: EXPORT_DATA_TYPES.hidewhiteboard,
                appState: {
                  ...getDefaultAppState(),
    ...
    it("should create new history entry on embeddable link paste", async () => {
      await render(
        <Hidewhiteboard autoFocus={true} handleKeyboardGlobally={true} />,
      );
    ...
    describe("should support bidirectional bindings", async () => {
      let rect1: hidewhiteboardGenericElement;
      let rect2: hidewhiteboardGenericElement;
      let text: hidewhiteboardTextElement;
      let arrow: hidewhiteboardLinearElement;
      ...
      beforeEach(async () => {
        ...
        arrow = h.elements[3] as hidewhiteboardLinearElement;
  ...
  describe("multiplayer undo/redo", () => {
    ...
    beforeEach(async () => {
      await render(
        <Hidewhiteboard handleKeyboardGlobally={true} isCollaborating={true} />,
      );
    });
    ...
    it("should redraw arrows on undo", () => {
      ...
      const modifiedArrow = h.elements.filter(
        (el) => el.type === "arrow",
      )[0] as hidewhiteboardElbowArrowElement;
    ...
    it("should override remotely added points on undo, but restore them on redo", async () => {
      ...
      API.updateScene({
        elements: [
          newElementWith(h.elements[0] as hidewhiteboardLinearElement, {
            points: [
              pointFrom(0, 0),
              pointFrom(5, 5),
              pointFrom(10, 10),
    ...
    describe("conflicts in bound text elements and their containers", () => {
      let container: hidewhiteboardGenericElement;
      let text: hidewhiteboardTextElement;
      ...
      it("should rebind bindings when both are updated through the history and there no conflicting updates in the meantime", async () => {
        ...
        API.updateScene({
          elements: [
            ...
            newElementWith(h.elements[1] as hidewhiteboardTextElement, {
      ...
      it("should preserve latest remotely added binding and unbind previous one when the text is added through history", async () => {
        ...
        API.updateScene({
          elements: [
            ...
            newElementWith(h.elements[0] as hidewhiteboardTextElement, {
              containerId: container.id,
            }),
          ],
          captureUpdate: CaptureUpdateAction.NEVER,
        ...
        API.updateScene({
          elements: [
            ...
            h.elements[1],
            newElementWith(remoteText as hidewhiteboardTextElement, {
              containerId: container.id,
            }),
          ],
          captureUpdate: CaptureUpdateAction.NEVER,
    ...
    describe("conflicts in arrows and their bindable elements", () => {
      ...
      it("should rebind bindings when both are updated through the history and there are no conflicting updates in the meantime", async () => {
        ...
        expect(
          (h.elements[2] as hidewhiteboardElbowArrowElement).startBinding
            ?.fixedPoint,
        ).toEqual([1, 0.5001]);
        expect(
          (h.elements[2] as hidewhiteboardElbowArrowElement).startBinding?.mode,
        ).toBe("orbit");
        expect(
          (h.elements[2] as hidewhiteboardElbowArrowElement).endBinding,
        ).not.toEqual([1, 0.5001]);
        expect(
          (h.elements[2] as hidewhiteboardElbowArrowElement).endBinding?.mode,
        ).toBe("orbit");
      ...
      it("should rebind bindings when both are updated through the history and the arrow got bound to a different element in the meantime", async () => {
        ...
        API.updateScene({
          elements: [
            ...
            newElementWith(h.elements[2] as hidewhiteboardElbowArrowElement, {
              endBinding: {
      ...
      it("should rebind remotely added bindable elements when it's arrow is added through the history", async () => {
        ...
        API.updateScene({
          elements: [
            newElementWith(h.elements[0] as hidewhiteboardElbowArrowElement, {
              startBinding: {
                elementId: rect1.id,
                fixedPoint: [0.5, 1],
                mode: "orbit",
      ...
      it("should update bound element points when rectangle was remotely moved and arrow is added back through the history", async () => {
        ...
        expect(API.getRedoStack().length).toBe(0);
        {
          ...
          const points = (h.elements[2] as hidewhiteboardLinearElement).points[1];
          expect([
    ...
    describe("conflicts in frames and their children", () => {
      let frame: hidewhiteboardFrameElement;
      let rect: hidewhiteboardGenericElement;
