import { promiseTry, LOCAL_FONT_PROTOCOL } from "@hidewhiteboard/common";

import { subsetWoff2GlyphsByCodepoints } from "../subset/subset-main";

type DataURL = string;

export class hidewhiteboardFontFace {
  ...
  private static readonly ASSETS_FALLBACK_URL = `https://esm.sh/${
    import.meta.env.PKG_NAME
      ...
      : "@hidewhiteboard/hidewhiteboard" // fallback to the latest package version (i.e. for app)
  }/dist/prod/`;

  constructor(family: string, uri: string, descriptors?: FontFaceDescriptors) {
    this.urls = hidewhiteboardFontFace.createUrls(uri);

    const sources = this.urls
      .map((url) => `url(${url}) ${hidewhiteboardFontFace.getFormat(url)}`)
      .join(", ");
  ...
  private static createUrls(uri: string): URL[] | DataURL[] {
    ...
    // absolute assets paths, which are found in tests and hidewhiteboard-app build, won't work with base url, so we are stripping initial slash away
    const assetUrl: string = uri.replace(/^\/+/, "");
    const urls: URL[] = [];

    if (typeof window.HIDEWHITEBOARD_ASSET_PATH === "string") {
      const normalizedBaseUrl = this.normalizeBaseUrl(
        window.HIDEWHITEBOARD_ASSET_PATH,
      );

      urls.push(new URL(assetUrl, normalizedBaseUrl));
    } else if (Array.isArray(window.HIDEWHITEBOARD_ASSET_PATH)) {
      window.HIDEWHITEBOARD_ASSET_PATH.forEach((path) => {
    ...
    urls.push(new URL(assetUrl, hidewhiteboardFontFace.ASSETS_FALLBACK_URL));
