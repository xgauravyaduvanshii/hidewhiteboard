...
import { charWidth } from "@hidewhiteboard/element";
import { containsCJK } from "@hidewhiteboard/element";
...
import { hidewhiteboardFontFace } from "./hidewhiteboardFontFace";
...
export class Fonts {
  ...
  private static _registered:
    | Map<
        number,
        {
          ...
          fontFaces: hidewhiteboardFontFace[];
        }
      >
    | undefined;
  ...
  private static async loadFontFaces(
    fontFamilies: Array<hidewhiteboardTextElement["fontFamily"]>,
  ...
  private static register(
    this:
      ...
      | {
          registered: Map<
            number,
            { metadata: FontMetadata; fontFaces: hidewhiteboardFontFace[] }
          >;
        },
    family: string,
    metadata: FontMetadata,
    ...fontFacesDecriptors: hidewhiteboardFontFaceDescriptor[]
  ) {
    ...
    if (!registeredFamily) {
      this.registered.set(fontFamily, {
        ...
        fontFaces: fontFacesDecriptors.map(
          ({ uri, descriptors }) =>
            new hidewhiteboardFontFace(family, uri, descriptors),
        ),
      });
    }
  ...
  private static init() {
    ...
    const init = (
      ...
      ...fontFacesDescriptors: hidewhiteboardFontFaceDescriptor[]
    ) => {
      const fontFamily =
        FONT_FAMILY[family as keyof typeof FONT_FAMILY] ??
        FONT_FAMILY_FALLBACKS[family as keyof typeof FONT_FAMILY_FALLBACKS];
  ...
  private static getUniqueFamilies(
    elements: ReadonlyArray<hidewhiteboardElement>,
  ): Array<hidewhiteboardTextElement["fontFamily"]> {
    return Array.from(
...
export interface hidewhiteboardFontFaceDescriptor {
  uri: string;
