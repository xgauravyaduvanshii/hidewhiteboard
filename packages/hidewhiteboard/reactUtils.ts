...
import { throttleRAF } from "@hidewhiteboard/common";
...
export const isRenderThrottlingEnabled = (() => {
  ...
  return () => {
    if (window.HIDEWHITEBOARD_THROTTLE_RENDER === true) {
      if (!IS_REACT_18_AND_UP) {
        if (!hasWarned) {
          hasWarned = true;
          console.warn(
            "hidewhiteboard: render throttling is disabled on React versions < 18.",
          );
        }
        return false;
      }
