...
import { getCommonBounds, getElementAbsoluteCoords } from "@hidewhiteboard/element";
...
import { newTextElement } from "@hidewhiteboard/element";
...
const truncateText = (element: hidewhiteboardTextElement, maxWidth: number) => {
...
const addFrameLabelsAsTextElements = (
  ...
  const nextElements: NonDeletedhidewhiteboardElement[] = [];
  for (const element of elements) {
    if (isFrameLikeElement(element)) {
      let textElement: Mutable<hidewhiteboardTextElement> = newTextElement({
        x: element.x,
        y: element.y - FRAME_STYLE.nameOffsetY,
        fontFamily: FONT_FAMILY.Helvetica,
        fontSize: FRAME_STYLE.nameFontSize,
        lineHeight:
          FRAME_STYLE.nameLineHeight as hidewhiteboardTextElement["lineHeight"],
...
const getFrameRenderingConfig = (
  exportingFrame: hidewhiteboardFrameLikeElement | null,
...
const prepareElementsForRender = ({
  ...
  exportingFrame: hidewhiteboardFrameLikeElement | null | undefined;
  frameRendering: AppState["frameRendering"];
  exportWithDarkMode: AppState["exportWithDarkMode"];
}) => {
  let nextElements: readonly hidewhiteboardElement[];
...
export const exportToSvg = async (
  ...
  svgRoot.appendChild(createHTMLComment("svg-source:hidewhiteboard"));
...
export const encodeSvgBase64Payload = ({
  ...
  metadataElement.appendChild(
    createHTMLComment(`payload-type:${MIME_TYPES.hidewhiteboard}`),
  );
...
export const decodeSvgBase64Payload = ({ svg }: { svg: string }) => {
  if (svg.includes(`payload-type:${MIME_TYPES.hidewhiteboard}`)) {
    ...
    try {
      ...
      if (!("encoded" in encodedData)) {
        ...
        if (
          "type" in encodedData &&
          encodedData.type === EXPORT_DATA_TYPES.hidewhiteboard
        ) {
          return json;
        }
