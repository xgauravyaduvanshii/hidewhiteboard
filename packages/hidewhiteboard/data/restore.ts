import { isFiniteNumber, pointFrom } from "@hidewhiteboard/math";
...
import { normalizeFixedPoint } from "@hidewhiteboard/element";
import {
  updateElbowArrowPoints,
  validateElbowPoints,
} from "@hidewhiteboard/element";
import { LinearElementEditor } from "@hidewhiteboard/element";
import { bumpVersion } from "@hidewhiteboard/element";
import { getContainerElement } from "@hidewhiteboard/element";
import { detectLineHeight } from "@hidewhiteboard/element";
import {
...
import { syncInvalidIndices } from "@hidewhiteboard/element";

import { refreshTextDimensions } from "@hidewhiteboard/element";

import { getNormalizedDimensions } from "@hidewhiteboard/element";

import { isInvisiblySmallElement } from "@hidewhiteboard/element";
...
import type { MarkOptional, Mutable } from "@hidewhiteboard/common/utility-types";
...
export const AllowedhidewhiteboardActiveTools: Record<
...
const repairBinding = <T extends hidewhiteboardArrowElement>(
  element: T,
  ...
  try {
    ...
    if (isElbowArrow(element)) {
      const fixedPointBinding:
        | hidewhiteboardElbowArrowElement["startBinding"]
        | hidewhiteboardElbowArrowElement["endBinding"] = {
        ...binding,
    ...
    const targetBoundElement =
      (targetElementsMap.get(binding.elementId) as hidewhiteboardBindableElement) ||
      undefined;
    const boundElement =
      targetBoundElement ||
...
const restoreElementWithProperties = <
  T extends Required<Omit<hidewhiteboardElement, "customData">> & {
    customData?: hidewhiteboardElement["customData"];
    /** @deprecated */
    boundElementIds?: readonly hidewhiteboardElement["id"][];
    /** @deprecated */
    strokeSharpness?: StrokeRoundness;
  },
  K extends Pick<T, keyof Omit<Required<T>, keyof hidewhiteboardElement>>,
>(
  element: T,
  extra: Pick<
    T,
    ...
    keyof K
  > &
    Partial<Pick<hidewhiteboardElement, "type" | "x" | "y" | "customData">>,
): T => {
  const base: Pick<T, keyof hidewhiteboardElement> = {
...
export const restoreElement = (
  ...
  element: Exclude<hidewhiteboardElement, hidewhiteboardSelectionElement>,
  ...
  switch (element.type) {
    case "text":
      // temp fix: cleanup legacy obsidian-hidewhiteboard attribute else it'll
    ...
    case "arrow": {
      ...
      const base = {
        ...
        x,
        y,
        elbowed: (element as hidewhiteboardArrowElement).elbowed,
        ...getSizeFromPoints(points),
      };

      // TODO: Separate arrow from linear element
      const restoredElement = isElbowArrow(element)
        ? restoreElementWithProperties(element as hidewhiteboardElbowArrowElement, {
            ...base,
            elbowed: true,
            fixedSegments:
        ...
        : restoreElementWithProperties(element as hidewhiteboardArrowElement, base);
...
const repairContainerElement = (
  container: Mutable<hidewhiteboardElement>,
  elementsMap: Map<string, Mutable<hidewhiteboardElement>>,
) => {
  if (container.boundElements) {
    ...
    const boundIds = new Set<hidewhiteboardElement["id"]>();
...
const repairBoundElement = (
  boundElement: Mutable<hidewhiteboardTextElement>,
  elementsMap: Map<string, Mutable<hidewhiteboardElement>>,
) => {
...
const repairFrameMembership = (
  element: Mutable<hidewhiteboardElement>,
  elementsMap: Map<string, Mutable<hidewhiteboardElement>>,
) => {
  if (element.frameId) {
...
export const restoreElements = <T extends hidewhiteboardElement>(
  ...
  opts?:
    ...
    | undefined,
): CombineBrandsIfNeeded<T, OrderedhidewhiteboardElement> => {
  ...
  const restoredElements = syncInvalidIndices(
    (targetElements || []).reduce((elements, element) => {
      ...
      let migratedElement: hidewhiteboardElement | null;
      try {
  ...
  for (const element of restoredElements) {
    ...
    if (isLinearElement(element)) {
      if (
        ...
        (element as Mutable<hidewhiteboardLinearElement>).startBinding = null;
      }
      if (
        element.endBinding &&
        ...
        (element as Mutable<hidewhiteboardLinearElement>).endBinding = null;
      }
    }
  }
  ...
  return restoredElements.map((element) => {
    ...
    return element;
  }) as CombineBrandsIfNeeded<T, OrderedhidewhiteboardElement>;
};
 ...
 *
 * See https://github.com/xgauravyaduvanshii/hidewhiteboard/issues/3795
 *
 ...
 * updates).
 */
export const bumpElementVersions = <T extends hidewhiteboardElement>(
    ...
    activeTool: {
      ...updateActiveTool(
        ...
        nextAppState.activeTool.type &&
          AllowedhidewhiteboardActiveTools[nextAppState.activeTool.type]
