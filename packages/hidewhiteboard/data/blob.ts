...
import type { hidewhiteboardElement, FileId } from "@hidewhiteboard/element/types";
...
import { isValidhidewhiteboardData, isValidLibrary } from "./json";
import {
  restoreAppState,
  restoreElements,
  restoreLibraryItems,
...
export const getMimeType = (blob: Blob | string): string => {
  ...
  if (/\.(hidewhiteboard|json)$/.test(name)) {
  ...
  } else if (/\.hidewhiteboardlib$/.test(name)) {
    return MIME_TYPES.hidewhiteboardlib;
  }
  return "";
};
...
export const getFileHandleType = (handle: FileSystemHandle | null) => {
  ...
  return handle.name.match(/\.(json|hidewhiteboard|png|svg)$/)?.[1] || null;
};
...
export const loadSceneOrLibraryFromBlob = async (
  ...
  localElements: readonly hidewhiteboardElement[] | null,
  ...
  try {
    ...
    if (isValidhidewhiteboardData(data)) {
      return {
        type: MIME_TYPES.hidewhiteboard,
        data: {
      ...
      return {
        type: MIME_TYPES.hidewhiteboardlib,
        data,
      };
    }
 ...
 * Note: doesn't handle missing .hidewhiteboard/.hidewhiteboardlib extension  */
export const normalizeFile = async (file: File) => {
  ...
  if (file?.name?.endsWith(".hidewhiteboardlib")) {
    file = createFile(file, MIME_TYPES.hidewhiteboardlib, file.name);
  } else if (file?.name?.endsWith(".hidewhiteboard")) {
    file = createFile(file, MIME_TYPES.hidewhiteboard, file.name);
