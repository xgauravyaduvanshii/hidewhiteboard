...
import { hashElementsVersion, hashString } from "@hidewhiteboard/element";

import { getCommonBoundingBox } from "@hidewhiteboard/element";

import type { hidewhiteboardElement } from "@hidewhiteboard/element/types";

import type { MaybePromise } from "@hidewhiteboard/common/utility-types";
...
const ALLOWED_LIBRARY_URLS = [
  ...
  "raw.githubusercontent.com/hidewhiteboard/hidewhiteboard-libraries",
];
...
const isUniqueItem = (
  ...
  return !existingLibraryItems.find((libraryItem) => {
    ...
    // detect z-index difference by checking the hidewhiteboard elements
    // are in order
    return libraryItem.elements.every((libItemhidewhiteboardItem, idx) => {
      return (
        libItemhidewhiteboardItem.id === targetLibraryItem.elements[idx].id &&
        libItemhidewhiteboardItem.versionNonce ===
...
class Library {
  ...
  /** call on hidewhiteboard instance unmount */
  destroy = () => {
    this.updateQueue = [];
  ...
  // NOTE this is a high-level public API (exposed on hidewhiteboardAPI) with
...
export const distributeLibraryItemsOnSquareGrid = (
  ...
  const resElements: hidewhiteboardElement[] = [];
...
export const useHandleLibrary = (
  opts: {
    hidewhiteboardAPI: hidewhiteboardImperativeAPI | null;
    /**
     * Return `true` if the library install url should be allowed.
     * If not supplied, only the hidewhiteboard.com base domain is allowed.
     */
  ...
  useEffect(() => {
    ...
    // reset on editor remount (hidewhiteboardAPI changed)
    ...
    const importLibraryFromURL = async ({
      ...
      const shouldPrompt = idToken !== hidewhiteboardAPI.id;
      ...
      try {
        await hidewhiteboardAPI.updateLibrary({
    ...
    // important this useEffect only depends on hidewhiteboardAPI so it only reruns
    // on editor remounts (the hidewhiteboardAPI changes)
    hidewhiteboardAPI,
  ]);

  // This effect is run without hidewhiteboardAPI dependency so that host apps
      ...
      const unsubOnLibraryUpdate = onLibraryUpdateEmitter.on(
        async (update, nextLibraryItems) => {
          ...
          try {
            ...
            if (isLoaded && optsRef.current.hidewhiteboardAPI) {
              optsRef.current.hidewhiteboardAPI.updateScene({
                appState: {
