 ...
 * (but in semi-width and semi-height so it's more relevant to hidewhiteboard)
 *
 * the idea with pure shapes is so that we can provide collision and other geoemtric methods not depending on
 * the specifics of roughjs or elements in hidewhiteboard; instead, we can focus on the pure shapes themselves
 *
 * also included in this file are methods for converting an hidewhiteboard element or a Drawable from roughjs
 * to pure shapes
 */
import { pointsOnBezierCurves } from "points-on-curve";

import { invariant } from "@hidewhiteboard/common";
import {
  curve,
  lineSegment,
  pointFrom,
...
import type { Curve, LineSegment, Polygon, Radians } from "@hidewhiteboard/math";
