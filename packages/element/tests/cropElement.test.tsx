...
import { KEYS, cloneJSON } from "@hidewhiteboard/common";
...
import {
  ...
  actionFlipVertical,
} from "@hidewhiteboard/hidewhiteboard/actions";
...
import type { hidewhiteboardImageElement, ImageCrop } from "../src/types";
...
const generateRandomNaturalWidthAndHeight = (image: hidewhiteboardImageElement) => {
...
describe("Crop an image", () => {
  it("Cropping changes the dimension", async () => {
    const image = h.elements[0] as hidewhiteboardImageElement;
  ...
  it("Preserve aspect ratio", async () => {
    let image = h.elements[0] as hidewhiteboardImageElement;
    const initialWidth = image.width;
    const initialHeight = image.height;
