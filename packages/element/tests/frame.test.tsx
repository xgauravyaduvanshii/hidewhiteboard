...
import { Keyboard, Pointer } from "@hidewhiteboard/hidewhiteboard/tests/helpers/ui";
import {
  getCloneByOrigId,
  render,
} from "@hidewhiteboard/hidewhiteboard/tests/test-utils";

import type { hidewhiteboardElement } from "../src/types";
...
describe("adding elements to frames", () => {
  ...
  function expectEqualIds(expected: hidewhiteboardElement[]) {
    expect(h.elements.map((x) => x.id)).toEqual(expected.map((x) => x.id));
  }

  let frame: hidewhiteboardElement;
  let rect1: hidewhiteboardElement;
  let rect2: hidewhiteboardElement;
  let rect3: hidewhiteboardElement;
  let rect4: hidewhiteboardElement;
  let text: hidewhiteboardElement;
  let arrow: hidewhiteboardElement;
