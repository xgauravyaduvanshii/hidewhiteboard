 ...
 */
export const duplicateElement = <TElement extends hidewhiteboardElement>(
...
export const duplicateElements = (
  ...
  // loop over them.
  const processedIds = new Map<hidewhiteboardElement["id"], true>();
  const groupIdMap = new Map();
  const duplicatedElements: hidewhiteboardElement[] = [];
  const origElements: hidewhiteboardElement[] = [];
  ...
  const duplicateIdToOrigElement = new Map<
    ...
    hidewhiteboardElement
  >();
  const duplicateElementsMap = new Map<string, hidewhiteboardElement>();
  ...
  const elementsWithDuplicates: hidewhiteboardElement[] = elements.slice();
  ...
  const copyElements = <T extends hidewhiteboardElement | hidewhiteboardElement[]>(
    element: T,
  ): T extends hidewhiteboardElement[]
    ? hidewhiteboardElement[]
    : hidewhiteboardElement | null => {
    const elements = castArray(element);

    const _newElements = elements.reduce(
      (acc: hidewhiteboardElement[], element) => {
    ...
    ) as T extends hidewhiteboardElement[]
      ? hidewhiteboardElement[]
      : hidewhiteboardElement | null;
  };
  ...
  const insertBeforeOrAfterIndex = (
    index: number,
    elements: hidewhiteboardElement | null | hidewhiteboardElement[],
  ) => {
    if (!elements) {
      return;
    }
...
// Simplified deep clone for the purpose of cloning hidewhiteboardElement.
//
...
// passing hidewhiteboardElement as the top-level argument).
const _deepCopyElement = (val: any, depth: number = 0) => {
  ...
  // don't support them on hidewhiteboard elements yet. If we do, we need to make
...
/**
 * Clones hidewhiteboardElement data structure. Does not regenerate id, nonce, or
 ...
 * Typed arrays and other non-null objects.
 */
export const deepCopyElement = <T extends hidewhiteboardElement>(
  val: T,
): Mutable<T> => {
