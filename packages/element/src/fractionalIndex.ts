 ...
 * - it's necessary to always keep the array support for backwards compatibility (restore) - old scenes, old libraries, supporting multiple hidewhiteboard versions etc.
...
export const validateFractionalIndices = (
  elements: readonly hidewhiteboardElement[],
  {
    ...
    ignoreLogs?: true;
    reconciliationContext?: {
      localElements: ReadonlyArray<hidewhiteboardElement>;
      remoteElements: ReadonlyArray<hidewhiteboardElement>;
    };
  },
) => {
  const errorMessages = [];
  const stringifyElement = (element: hidewhiteboardElement | void) =>
...
export const syncMovedIndices = (
  ...
  return elements as OrderedhidewhiteboardElement[];
};
 ...
 */
const getInvalidIndicesGroups = (elements: readonly hidewhiteboardElement[]) => {
  const indicesGroups: number[][] = [];

  // once we find lowerBound / upperBound, it cannot be lower than that, so we cache it for better perf.
  let lowerBound: hidewhiteboardElement["index"] | undefined = undefined;
  let upperBound: hidewhiteboardElement["index"] | undefined = undefined;
  ...
  const getLowerBound = (
    index: number,
  ): [hidewhiteboardElement["index"] | undefined, number] => {
...
const isValidFractionalIndex = (
  index: hidewhiteboardElement["index"] | undefined,
  predecessor: hidewhiteboardElement["index"] | undefined,
  successor: hidewhiteboardElement["index"] | undefined,
) => {
  if (!index) {
    return false;
  }
...
): element is OrderedhidewhiteboardElement => {
