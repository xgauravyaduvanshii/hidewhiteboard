...
import type { Curve, LineSegment, LocalPoint } from "@hidewhiteboard/math";
...
const ElementShapesCache = new WeakMap<
  hidewhiteboardElement,
  { version: hidewhiteboardElement["version"]; shapes: Map<number, ElementShape> }
>();

const getElementShapesCacheEntry = <T extends hidewhiteboardElement>(
  element: T,
  offset: number,
...
const setElementShapesCacheEntry = <T extends hidewhiteboardElement>(
  element: T,
  shape: ElementShape,
  offset: number,
) => {
...
export const getCornerRadius = (x: number, element: hidewhiteboardElement) => {
  if (
