import { pointFrom, type LocalPoint } from "@hidewhiteboard/math";
...
import type { MarkOptional } from "@hidewhiteboard/common/utility-types";
  ...
  label?: {
    ...
    | (
        | (
            | {
                type: Exclude<
                  hidewhiteboardBindableElement["type"],
                  | "image"
                  | "text"
                  | "frame"
                  | "magicframe"
                  ...
                  | "iframe"
                >;
                id?: hidewhiteboardGenericElement["id"];
              }
            | {
                id: hidewhiteboardGenericElement["id"];
                type?: Exclude<
        ...
        | ((
            ...
            | {
                type?: "text";
                id: hidewhiteboardTextElement["id"];
                text: string;
              }
          ) &
            Partial<hidewhiteboardTextElement>)
      ) &
        MarkOptional<ElementConstructorOpts, "x" | "y">;
  start?:
    | (
        ...
        MarkOptional<ElementConstructorOpts, "x" | "y">;
} & Partial<hidewhiteboardLinearElement>;

export type ValidContainer =
  | {
      type: Exclude<hidewhiteboardGenericElement["type"], "selection">;
      id?: hidewhiteboardGenericElement["id"];
      label?: {
        text: string;
        fontSize?: number;
...
export type hidewhiteboardElementSkeleton =
  | Extract<
      Exclude<hidewhiteboardElement, hidewhiteboardSelectionElement>,
      hidewhiteboardIframeLikeElement | hidewhiteboardFreeDrawElement
    >
  | ({
      type: Extract<hidewhiteboardLinearElement["type"], "line">;
      x: number;
      y: number;
    } & Partial<hidewhiteboardLinearElement>)
  | ValidContainer
  ...
  | ({
      type: Extract<hidewhiteboardImageElement["type"], "image">;
      x: number;
      y: number;
      fileId: FileId;
    } & Partial<hidewhiteboardImageElement>)
  | ({
      type: "frame";
      children: readonly hidewhiteboardElement["id"][];
      name?: string;
    } & Partial<hidewhiteboardFrameElement>)
  | ({
      type: "magicframe";
      children: readonly hidewhiteboardElement["id"][];
      name?: string;
    } & Partial<hidewhiteboardMagicFrameElement>);
...
const bindTextToContainer = (
  container: hidewhiteboardElement,
  textProps: { text: string } & MarkOptional<ElementConstructorOpts, "x" | "y">,
  scene: Scene,
) => {
  const textElement: hidewhiteboardTextElement = newTextElement({
    x: 0,
    y: 0,
...
const bindLinearElementToElement = (
  linearElement: hidewhiteboardArrowElement,
  start: ValidLinearElement["start"],
  end: ValidLinearElement["end"],
  elementStore: ElementStore,
  scene: Scene,
): {
  linearElement: hidewhiteboardLinearElement;
  startBoundElement?: hidewhiteboardElement;
  endBoundElement?: hidewhiteboardElement;
} => {
  ...
  if (start) {
    ...
    if (startType) {
      ...
      bindBindingElement(
        linearElement,
        startBoundElement as hidewhiteboardBindableElement,
        "orbit",
        "start",
        scene,
      );
  ...
  if (end) {
    ...
    if (endType) {
      ...
      bindBindingElement(
        linearElement,
        endBoundElement as hidewhiteboardBindableElement,
        "orbit",
        "end",
        scene,
      );
...
class ElementStore {
  hidewhiteboardElements = new Map<string, hidewhiteboardElement>();

  add = (ele?: hidewhiteboardElement) => {
    if (!ele) {
      return;
    }

    this.hidewhiteboardElements.set(ele.id, ele);
  };

  getElements = () => {
    return syncInvalidIndices(Array.from(this.hidewhiteboardElements.values()));
  };
  ...
  getElement = (id: string) => {
    return this.hidewhiteboardElements.get(id);
  };
}

export const convertTohidewhiteboardElements = (
  elementsSkeleton: hidewhiteboardElementSkeleton[] | null,
  ...
  const elementsWithIds = new Map<string, hidewhiteboardElementSkeleton>();
  const oldToNewElementIdMap = new Map<string, string>();

  // Create individual elements
  for (const element of elements) {
    let hidewhiteboardElement: hidewhiteboardElement;
    ...
    switch (element.type) {
      ...
      case "diamond": {
        ...
        hidewhiteboardElement = newElement({
          ...element,
          width,
          height,
        });
      ...
      case "line": {
        ...
        hidewhiteboardElement = newLinearElement({
          width,
          height,
      ...
      case "arrow": {
        const width = element.width || DEFAULT_LINEAR_ELEMENT_PROPS.width;
        const height = element.height || DEFAULT_LINEAR_ELEMENT_PROPS.height;
        hidewhiteboardElement = newArrowElement({
          width,
          height,
          endArrowhead: "arrow",
          points: [pointFrom(0, 0), pointFrom(width, height)],
        ...
        Object.assign(
          hidewhiteboardElement,
          getSizeFromPoints(hidewhiteboardElement.points),
        );
        break;
      }
      case "text": {
        ...
        hidewhiteboardElement = newTextElement({
          width: metrics.width,
      ...
      case "image": {
        hidewhiteboardElement = newImageElement({
      ...
      case "frame": {
        hidewhiteboardElement = newFrameElement({
          x: 0,
          y: 0,
          ...element,
        });
      ...
      case "magicframe": {
        hidewhiteboardElement = newMagicFrameElement({
          x: 0,
          y: 0,
          ...element,
        });
    ...
    const existingElement = elementStore.getElement(hidewhiteboardElement.id);
    if (existingElement) {
      console.error(`Duplicate id found for ${hidewhiteboardElement.id}`);
    } else {
      elementStore.add(hidewhiteboardElement);
      elementsWithIds.set(hidewhiteboardElement.id, element);
      if (originalId) {
        oldToNewElementIdMap.set(originalId, hidewhiteboardElement.id);
      }
    }
  }
  ...
  for (const [id, element] of elementsWithIds) {
    const hidewhiteboardElement = elementStore.getElement(id)!;
        ...
        if (element.label?.text) {
          ...
          switch (element.type) {
            case "arrow": {
              ...
              const { linearElement, startBoundElement, endBoundElement } =
                bindLinearElementToElement(
                  hidewhiteboardElement as hidewhiteboardArrowElement,
                  start,
                  end,
                  elementStore,
                  scene,
  ...
  // Once all the hidewhiteboard elements are created, we can add frames since we
  ...
  for (const [id, element] of elementsWithIds) {
    ...
    if (!frame) {
      throw new Error(`hidewhiteboard element with id ${id} doesn't exist`);
    }
    const childrenElements: hidewhiteboardElement[] = [];
