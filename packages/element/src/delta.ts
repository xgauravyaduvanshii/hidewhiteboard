      ...
      const selectedLinearElement =
        ...
        nextElements.has(insertedSelectedLinearElement.elementId)
          ? new LinearElementEditor(
              ...
              ) as NonDeleted<hidewhiteboardLinearElement>,
              nextElements,
...
type ElementPartial<TElement extends hidewhiteboardElement = hidewhiteboardElement> =
...
export class ElementsDelta implements DeltaContainer<SceneElementsMap> {
   ...
   */
  public static calculate<T extends OrderedhidewhiteboardElement>(
  ...
  public applyLatestChanges(
    ...
    const modifier =
      (
        prevElement: OrderedhidewhiteboardElement | undefined,
        nextElement: OrderedhidewhiteboardElement | undefined,
      ) =>
      (partial: ElementPartial, partialType: "deleted" | "inserted") => {
        let element: OrderedhidewhiteboardElement | undefined;
  ...
  public applyTo(
    ...
    let changedElements: Map<string, OrderedhidewhiteboardElement>;
  ...
  private static createApplier =
    ...
    (deltas: Record<string, Delta<ElementPartial>>) => {
      ...
      return Object.entries(deltas).reduce((acc, [id, delta]) => {
        ...
        return acc;
      }, new Map<string, OrderedhidewhiteboardElement>());
    };
      ...
      if (!element) {
        ...
        if (element) {
          ...
          element = newElementWith(
            { id, version: 1 } as OrderedhidewhiteboardElement,
            {
              ...partial,
            },
          );
  ...
  private static applyDelta(
    element: OrderedhidewhiteboardElement,
  ...
  private resolveConflicts(
    ...
    const nextAffectedElements = new Map<string, OrderedhidewhiteboardElement>();
    const updater = (
      ...
      const elementUpdates = updates as ElementUpdate<OrderedhidewhiteboardElement>;

      let affectedElement: OrderedhidewhiteboardElement;
  ...
  public static redrawElements(
    nextElements: SceneElementsMap,
    changedElements: Map<string, OrderedhidewhiteboardElement>,
  ) {
    try {
  ...
  private static redrawTextBoundingBoxes(
    scene: Scene,
    changed: Map<string, OrderedhidewhiteboardElement>,
  ) {
    const elements = scene.getNonDeletedElementsMap();
    const boxesToRedraw = new Map<
      string,
      { container: OrderedhidewhiteboardElement; boundText: hidewhiteboardTextElement }
    >();

    for (const element of changed.values()) {
      if (isBoundToContainer(element)) {
        const { containerId } = element as hidewhiteboardTextElement;
        const container = containerId ? elements.get(containerId) : undefined;

        if (container) {
          boxesToRedraw.set(container.id, {
            container,
            boundText: element as hidewhiteboardTextElement,
          });
        }
      }
      ...
      if (hasBoundTextElement(element)) {
        ...
        if (boundText) {
          boxesToRedraw.set(element.id, {
            container: element,
            boundText: boundText as hidewhiteboardTextElement,
          });
        }
      }
    }
      ...
      const deletedPoints =
        (
          deleted as ElementPartial<
            hidewhiteboardFreeDrawElement | hidewhiteboardLinearElement
          >
        ).points ?? [];
  ...
  private static stripIrrelevantProps(
    partial: Partial<OrderedhidewhiteboardElement>,
  ): ElementPartial {
