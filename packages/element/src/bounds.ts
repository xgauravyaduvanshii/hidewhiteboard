...
export class ElementBounds {
  private static boundsCache = new WeakMap<
    ...
    {
      bounds: Bounds;
      version: hidewhiteboardElement["version"];
    }
  >();
  ...
  if (isFreeDrawElement(element)) {
    ...
    if (isArrowElement(container)) {
      const { x, y } = LinearElementEditor.getBoundTextElementPosition(
        container,
        element as hidewhiteboardTextElementWithContainer,
        elementsMap,
      );
      return [
        x,
...
export const getElementLineSegments = (
  ...
  if (shape.type === "polycurve") {
    ...
    return getSegmentsOnEllipse(element as hidewhiteboardEllipseElement);
  }
...
const _isRectanguloidElement = (
  element: hidewhiteboardElement,
): element is hidewhiteboardRectanguloidElement => {
  return (
    element != null &&
...
const getSegmentsOnEllipse = (
  ellipse: hidewhiteboardEllipseElement,
): LineSegment<GlobalPoint>[] => {
 ...
 * Scene -> Scene coords, but in x1,x2,y1,y2 format.
 *
 * Rectangle here means any rectangular frame, not an hidewhiteboard element.
 */
...
export const getDiamondPoints = (element: hidewhiteboardElement) => {
...
export const getCommonBoundingBox = (
  elements:
    ...
    | readonly NonDeleted<hidewhiteboardElement>[],
): BoundingBox => {
  const [minX, minY, maxX, maxY] = getCommonBounds(elements);
  return {
    minX,
...
export const aabbForElement = (
  element: Readonly<hidewhiteboardElement>,
  elementsMap: ElementsMap,
