...
import type { hidewhiteboardArrowElement } from "../types";
...
describe("Test Transform", () => {
  it("should generate id unless opts.regenerateIds is set to false explicitly", () => {
    ...
    let data = convertTohidewhiteboardElements(
      elements as hidewhiteboardElementSkeleton[],
    );
    expect(data.length).toBe(1);
    expect(data[0].id).toBe("id0");

    data = convertTohidewhiteboardElements(
      elements as hidewhiteboardElementSkeleton[],
      opts,
    );
  ...
  it("should transform linear elements", () => {
    ...
    const hidewhiteboardElements = convertTohidewhiteboardElements(
      elements as hidewhiteboardElementSkeleton[],
      opts,
    );

    expect(hidewhiteboardElements.length).toBe(4);

    hidewhiteboardElements.forEach((ele) => {
      expect(ele).toMatchSnapshot({
        seed: expect.any(Number),
        versionNonce: expect.any(Number),
        id: expect.any(String),
  ...
  it("should transform to text containers when label provided", () => {
    ...
    expect(hidewhiteboardElements.length).toBe(12);
  ...
  it("should transform to labelled arrows when label provided for arrows", () => {
    ...
    expect(hidewhiteboardElements.length).toBe(8);
  ...
  describe("Test Frames", () => {
    const elements: hidewhiteboardElementSkeleton[] = [
      {
        type: "rectangle",
        x: 10,
        y: 10,
    ...
    it("should transform frames and update frame ids when regenerated", () => {
      const elementsSkeleton: hidewhiteboardElementSkeleton[] = [
        ...elements,
        {
          type: "frame",
          children: ["1", "2"],
    ...
    it("should consider user defined frame dimensions over calculated when provided", () => {
      ...
      const frame = hidewhiteboardElements.find((ele) => ele.type === "frame")!;
  ...
  describe("Test arrow bindings", () => {
    it("should bind arrows to shapes when start / end provided without ids", () => {
      ...
      expect(hidewhiteboardElements.length).toBe(4);
      const [arrow, text, rectangle, ellipse] = hidewhiteboardElements;
    ...
    it("should bind arrows to text when start / end provided without ids", () => {
      ...
      expect(hidewhiteboardElements.length).toBe(4);
      const [arrow, text1, text2, text3] = hidewhiteboardElements;
    ...
    it("should bind arrows to existing shapes when start / end provided with ids", () => {
      ...
      expect(hidewhiteboardElements.length).toBe(5);
    ...
    it("should bind arrows to existing elements if ids are correct", () => {
      ...
      expect(hidewhiteboardElements.length).toBe(4);
      const [, , arrow, text] = hidewhiteboardElements;
      expect(arrow).toMatchObject({
        type: "arrow",
        x: 255.5,
        y: 239,
    ...
    it("should bind when ids referenced before the element data", () => {
      ...
      expect(hidewhiteboardElements.length).toBe(2);
      const [arrow, rect] = hidewhiteboardElements;
      expect((arrow as hidewhiteboardArrowElement).endBinding).toStrictEqual({
        elementId: "rect-1",
  ...
  it("should not allow duplicate ids", () => {
    ...
    expect(hidewhiteboardElements.length).toBe(1);
    expect(hidewhiteboardElements[0]).toMatchSnapshot({
      seed: expect.any(Number),
      versionNonce: expect.any(Number),
    });
  ...
  it("should contains customData if provided", () => {
    ...
    const convertedElements = convertTohidewhiteboardElements(
      rawData as hidewhiteboardElementSkeleton[],
      opts,
    );
  ...
  it("should transform the elements correctly when linear elements have single point", () => {
    ...
    const hidewhiteboardElements = convertTohidewhiteboardElements(elements, opts);
    expect(hidewhiteboardElements.length).toBe(12);
    hidewhiteboardElements.forEach((ele) => {
      expect(ele).toMatchSnapshot({
        seed: expect.any(Number),
