import { arrayToMap, findIndex, findLastIndex } from "@hidewhiteboard/common";
...
const isOfTargetFrame = (element: hidewhiteboardElement, frameId: string) => {
...
const getIndicesToMove = (
  ...
  appState: AppState,
  elementsToBeMoved?: readonly hidewhiteboardElement[],
) => {
...
const getTargetIndexAccountingForBinding = (
  nextElement: hidewhiteboardElement,
  elements: readonly hidewhiteboardElement[],
...
const getContiguousFrameRangeElements = (
  allElements: readonly hidewhiteboardElement[],
  frameId: hidewhiteboardFrameLikeElement["id"],
) => {
  let rangeStart = -1;
  let rangeEnd = -1;
...
export const moveArrowAboveBindable = (
  ...
  scene: Scene,
  hit?: NonDeletedhidewhiteboardElement,
): readonly OrderedhidewhiteboardElement[] => {
  ...
  const bindableIds = [
    ...
    boundTextElement?.id,
    containerElement?.id,
  ].filter((id): id is NonDeletedhidewhiteboardElement["id"] => !!id);
...
const getTargetIndex = (
   ...
   */
  containingFrame: hidewhiteboardFrameLikeElement["id"] | null,
  scene: Scene,
) => {
  const sourceElement = elements[boundaryIndex];

  const indexFilter = (element: hidewhiteboardElement) => {
...
const getTargetElementsMap = <T extends hidewhiteboardElement>(
  elements: readonly T[],
  indices: number[],
) => {
  return indices.reduce((acc, index) => {
    ...
    return acc;
  }, new Map<string, hidewhiteboardElement>());
};
...
const shiftElementsByOne = (
  ...
  const selectedFrames = new Set<hidewhiteboardFrameLikeElement["id"]>(
    indicesToMove
...
const shiftElementsToEnd = (
  ...
  const targetElementsMap = getTargetElementsMap(elements, indicesToMove);
  const displacedElements: hidewhiteboardElement[] = [];
...
function shiftElementsAccountingForFrames(
  ...
  shiftFunction: (
    ...
    containingFrame: hidewhiteboardFrameLikeElement["id"] | null,
    elementsToBeMoved?: readonly hidewhiteboardElement[],
  ) => hidewhiteboardElement[] | readonly hidewhiteboardElement[],
) {
  ...
  const frameAwareContiguousElementsToMove: {
    regularElements: hidewhiteboardElement[];
    frameChildren: Map<hidewhiteboardFrameLikeElement["id"], hidewhiteboardElement[]>;
  } = { regularElements: [], frameChildren: new Map() };

  const fullySelectedFrames = new Set<hidewhiteboardFrameLikeElement["id"]>();
