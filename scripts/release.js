...
const PACKAGES = ["common", "math", "element", "hidewhiteboard"];
 ...
 * - yarn release                                 -> publishes `@hidewhiteboard` packages with "test" tag and "-[hash]" version suffix
 * - yarn release --tag=test                      -> same as above
 * - yarn release --tag=next                      -> publishes `@hidewhiteboard` packages with "next" tag and version "-[hash]" suffix
 * - yarn release --tag=next --non-interactive    -> skips interactive prompts (runs on CI/CD), otherwise same as above
 * - yarn release --tag=latest --version=0.19.0   -> publishes `@hidewhiteboard` packages with "latest" tag and version "0.19.0" & prepares changelog for the release
 *
...
const getArguments = () => {
  ...
  if (!version) {
    // set the next version based on the hidewhiteboard package version + commit hash
    const hidewhiteboardPackageVersion = require(getPackageJsonPath(
      "hidewhiteboard",
    )).version;

    const hash = getShortCommitHash();

    if (!hidewhiteboardPackageVersion.includes(hash)) {
      version = `${hidewhiteboardPackageVersion}-${hash}`;
    } else {
      // ensuring idempotency
      version = hidewhiteboardPackageVersion;
    }
  }
  ...
  for (const packageName of PACKAGES) {
    ...
    if (pkg.dependencies) {
      for (const dependencyName of PACKAGES) {
        if (!pkg.dependencies[`@hidewhiteboard/${dependencyName}`]) {
          continue;
        }

        pkg.dependencies[`@hidewhiteboard/${dependencyName}`] = nextVersion;
      }
    }
    ...
    rl.question(
      ...
      (answer) => {
        ...
        if (answer.toLowerCase() === "y") {
          execSync(`git add -u`);
          execSync(
            `git commit -m "chore: release @hidewhiteboard/hidewhiteboard@${nextVersion} 🎉"`,
          );
        } else {
          console.warn(
...
const buildPackages = () => {
  ...
  for (const packageName of PACKAGES) {
    console.info(`Building "@hidewhiteboard/${packageName}"...`);
...
const publishPackages = (tag, version) => {
  for (const packageName of PACKAGES) {
    ...
    console.info(
      `Published "@hidewhiteboard/${packageName}@${tag}" with version "${version}"! 🎉`,
    );
  }
};
