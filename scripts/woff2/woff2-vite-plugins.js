// define `HIDEWHITEBOARD_ASSET_PATH` as a SSOT
const OSS_FONTS_CDN = "https://hidewhiteboard.nyc3.cdn.digitaloceanspaces.com/oss/";
const OSS_FONTS_FALLBACK = "/";

/**
 * Custom vite plugin for auto-prefixing `HIDEWHITEBOARD_ASSET_PATH` woff2 fonts in `hidewhiteboard-app`.
 *
...
module.exports.woff2BrowserPlugin = () => {
  ...
  return {
    ...
    transform(code, id) {
      ...
      // but at the same time can't be easily prefixed with the `HIDEWHITEBOARD_ASSET_PATH` only for the `hidewhiteboard-app`
      if (!isDev && id.endsWith("/hidewhiteboard/fonts/fonts.css")) {
        return `/* WARN: The following content is generated during hidewhiteboard-app build */
      ...
      if (!isDev && id.endsWith("hidewhiteboard-app/index.html")) {
        return code.replace(
          "<!-- PLACEHOLDER:HIDEWHITEBOARD_APP_FONTS -->",
          `<script>
        // point into our CDN in prod, fallback to root (hidewhiteboard.com) domain in case of issues
