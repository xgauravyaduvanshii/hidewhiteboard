...
const hidewhiteboardDir = `${__dirname}/../packages/hidewhiteboard`;
const hidewhiteboardPackage = `${hidewhiteboardDir}/package.json`;
const pkg = require(hidewhiteboardPackage);
const lastVersion = pkg.version;
const existingChangeLog = fs.readFileSync(
  `${hidewhiteboardDir}/CHANGELOG.md`,
  "utf8",
);
...
const getCommitHashForLastVersion = async () => {
  try {
    const commitMessage = `"release @hidewhiteboard/hidewhiteboard"`;
...
const getLibraryCommitsSinceLastRelease = async () => {
  ...
  commitsSinceLastRelease.forEach((commit) => {
    ...
    if (prMatch) {
      ...
      const prMarkdown = `[#${prNumber}](https://github.com/xgauravyaduvanshii/hidewhiteboard/pull/${prNumber})`;
...
const updateChangelog = async (nextVersion) => {
  ...
  fs.writeFileSync(`${hidewhiteboardDir}/CHANGELOG.md`, updatedContent, "utf8");
};
