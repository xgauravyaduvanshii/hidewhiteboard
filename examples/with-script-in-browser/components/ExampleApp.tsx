...
import type { ImportedLibraryData } from "@hidewhiteboard/hidewhiteboard/data/types";
import type {
  NonDeletedhidewhiteboardElement,
  Theme,
} from "@hidewhiteboard/hidewhiteboard/element/types";
import type {
  AppState,
  BinaryFileData,
  ...
  Gesture,
  LibraryItems,
  PointerDownState as hidewhiteboardPointerDownState,
...
export interface AppProps {
  appTitle: string;
  useCustom: (api: hidewhiteboardImperativeAPI | null, customArgs?: any[]) => void;
  customArgs?: any[];
...
export default function ExampleApp({
  ...
  const [hidewhiteboardAPI, sethidewhiteboardAPI] =
    useState<hidewhiteboardImperativeAPI | null>(null);

  useCustom(hidewhiteboardAPI, customArgs);
    ...
    const fetchData = async () => {
      ...
      reader.onload = function () {
        ...
        initialStatePromiseRef.current.promise.resolve({
          ...initialData,
          elements: convertTohidewhiteboardElements(initialData.elements),
        });
        hidewhiteboardAPI.addFiles(imagesArray);
      };
    };
    fetchData();
  }, [hidewhiteboardAPI, convertTohidewhiteboardElements, MIME_TYPES]);

  const renderhidewhiteboard = (children: React.ReactNode) => {
    const hidewhiteboard: any = Children.toArray(children).find(
      (child) =>
        React.isValidElement(child) &&
        typeof child.type !== "string" &&
        //@ts-ignore
        child.type.displayName === "hidewhiteboard",
    );
    if (!hidewhiteboard) {
      return;
    }
    const newElement = cloneElement(
      hidewhiteboard,
      {
        hidewhiteboardAPI: (api: hidewhiteboardImperativeAPI) => sethidewhiteboardAPI(api),
        initialData: initialStatePromiseRef.current.promise,
        onChange: (
          elements: NonDeletedhidewhiteboardElement[],
          state: AppState,
        ) => {
  ...
  const loadSceneOrLibrary = async () => {
    const file = await fileOpen({ description: "hidewhiteboard or library file" });
    const contents = await loadSceneOrLibraryFromBlob(file, null, null);
    if (contents.type === MIME_TYPES.hidewhiteboard) {
      hidewhiteboardAPI?.updateScene(contents.data as any);
  ...
  const updateScene = () => {
    ...
    hidewhiteboardAPI?.updateScene(sceneData);
  };
  ...
  const onCopy = async (type: "png" | "svg" | "json") => {
    ...
    await exportToClipboard({
      elements: hidewhiteboardAPI.getSceneElements(),
      appState: hidewhiteboardAPI.getAppState(),
      files: hidewhiteboardAPI.getFiles(),
      type,
    });
  ...
  const onPointerDown = (
    ...
    pointerDownState: hidewhiteboardPointerDownState,
  ) => {
  ...
  const onPointerUpFromPointerDownHandler = (
    ...
    return withBatchedUpdates((event) => {
      ...
      hidewhiteboardAPI?.setActiveTool({ type: "selection" });
      ...
      return (
        <div
          ...
          onPointerDown={(event) => {
            ...
            hidewhiteboardAPI?.setActiveTool({
              type: "custom",
              customType: "comment",
            });
          }}
  ...
  const renderMenu = () => {
    return (
      <MainMenu>
        ...
        />
        <MainMenu.Group title="hidewhiteboard links">
  ...
  return (
    <div className="App" ref={appRef}>
      ...
      <ExampleSidebar>
            ...
            <input
              ...
              onChange={() => {
                if (!isCollaborating) {
                  ...
                  hidewhiteboardAPI?.updateScene({ collaborators });
                } else {
        ...
        <div className="hidewhiteboard-wrapper">
          {renderhidewhiteboard(children)}
        ...
        <div className="export-wrapper button-wrapper">
          ...
          <button
            ...
            onClick={() => {
              ...
              hidewhiteboardAPI.scrollToContent(elements[0], {
                fitToViewport: true,
              });
            }}
          >
